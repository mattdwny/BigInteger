...
Position* DLLProjectList::before(const Position * p)
{
    DLLNode * node = dynamic_cast<DLLNode*>(node);
}

Position* DLLProjectList::after(const Position * p)
{
    DLLNode * node = dynamic_cast<DLLNode*>(node);
}

bool DLLProjectList::isFirst(const Position * p)
{
    DLLNode * node = dynamic_cast<DLLNode*>(node);
}

bool DLLProjectList::isLast(const Position * p)
{
    DLLNode * node = dynamic_cast<DLLNode*>(node);
}
